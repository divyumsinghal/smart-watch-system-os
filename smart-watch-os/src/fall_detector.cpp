...
fall_detector::fall_detector(DFRobot_I2C_Multiplexer* Mux_main, int port)
    : Mux(Mux_main), port_fall_detector(port) {
...
bool fall_detector::now_moving() {
  Mux->selectPort(port_fall_detector);

  float aX, aY, aZ, gX, gY, gZ, aSum;
